...
cardImg0.src = "graphics/cardback.gif";
var cardImg1 = new Image();
cardImg1.src = "graphics/jack.gif";
var cardImg2 = new Image();
cardImg2.src = "graphics/queen.gif";
var cardImg3 = new Image();
cardImg3.src = "graphics/king.gif";
...
function cardCreateNode() {
  ...
  if (this.rank == "J") tempNode.src = "graphics/jack.gif";
  if (this.rank == "Q") tempNode.src = "graphics/queen.gif";
  if (this.rank == "K") tempNode.src = "graphics/king.gif";
...
function initGame() {
  ...
  for (i = 0; i < player.length; i++) player[i] = new Hand("player" + i);
}
...
function handReset() {
  ...
  if (this.resultTextNode) this.resultTextNode.nodeValue = "\u00a0";
}
...
function handAddCard(card, down) {
  ...
  if (this.cards.length % this.rollEvery == 0) this.top = 0;
  else this.top += this.topIncr;
}
  ...
  for (i = 0; i < this.cards.length; i++)
    if (this.cards[i].rank == "A") total++;
      ...
      if (
        ...
        total += 10;
      else total += parseInt(this.cards[i].rank, 10);
    }

  // Change as many ace values to 11 as possible.

  for (i = 0; i < this.cards.length; i++)
    if (this.cards[i].rank == "A" && total <= 11) total += 10;
...
function startRound() {
  ...
  for (i = 0; i < player.length; i++) {
    player[i].reset();
    if (i > 0) player[i].fieldNode.style.display = "none";
  }
...
function dealRound() {
  ...
  } else player[0].scoreTextNode.nodeValue = player[0].getScore();
...
function playRound() {
  ...
  if (dealer.cards[1].rank == "A") offerInsurance();
  ...
  if (canSplit()) document.forms["controls"].elements["split"].disabled = false;
...
function playerHit() {
  ...
  if (p > 21) {
    ...
    startNextHand();
    return;
  } else player[n].scoreTextNode.nodeValue = p;
...
function startNextHand() {
  ...
  if (curPlayerHand >= numPlayerHands) {
    ...
    if (player[curPlayerHand].split) setTimeout(playerHit, dealTimeDelay);
  }
}
...
function startDealer() {
  ...
  for (i = 0; i < numPlayerHands; i++)
    if (player[i].getScore() <= 21) allBusts = false;
  if (allBusts) {
    endRound();
    return;
  }
...
function playDealer() {
  ...
  if (d > 21) dealer.scoreTextNode.nodeValue = "Busted (" + d + ")";
...
function endRound() {
  ...
  dealer.cardsNode.firstChild.firstChild.style.visibility = "";
  d = dealer.getScore();
  if (!dealer.blackjack && d <= 21) dealer.scoreTextNode.nodeValue = d;
  ...
  for (i = 0; i < numPlayerHands; i++) {
    ...
    if (player[i].surrender) {
      ...
      (player[i].blackjack && !dealer.blackjack) ||
      (p <= 21 && d > 21) ||
      (p <= 21 && p > d)
    ) {
      ...
      if (player[i].blackjack) tmp += player[i].bet / 2;
...
function canSplit() {
  ...
  if (numPlayerHands > maxSplits) return false;

  // Check for a pair.

  n = curPlayerHand;
  if (player[n].cards[0].rank == player[n].cards[1].rank) return true;

  // Also, allow tens and face cards to match as a pair.

  if (
    (player[n].cards[0].rank == "10" ||
...
function updateBetDisplay(n) {
  ...
  if (player[n]) {
    if (player[n].bet != null) s = "Bet: " + formatDollar(player[n].bet);
    else s = "\u00a0";
...
function EnableBetButtons() {
  ...
  document.forms["controls"].elements["increase"].disabled =
    defaultBet >= maxBet;
  document.forms["controls"].elements["decrease"].disabled =
    defaultBet <= minBet;
}
...
function addClassName(el, name) {
  ...
  if (el.className.length > 0) name = " " + name;
  el.className += name;
}
